...
class CIRCODataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[str, Path], split: Literal['val', 'test'],
                 mode: Literal['relative', 'classic'], preprocess: callable = None):
        """
        Args:
        ...
        self.img_paths = [dataset_path / 'COCO2017_unlabeled' / "unlabeled2017" / img_info["file_name"] for img_info in imgs_info["images"]]
...
class ImageNetDataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'], preprocess: callable = None):
        dataset_path = Path(dataset_path)
        self.dataset_path = dataset_path
        self.split = split
        self.preprocess = preprocess
